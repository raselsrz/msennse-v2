            ...
            <div class="card shadow border-0 mb-7">
                ...
                <div class="card-header">
                    <div class="row align-items-center">
                        <div class="col">
                            <div class="mb-3">
                                <input type="text" id="searchInput" class="form-control" placeholder="Search Here" required>
                            </div>
                        </div>
                    </div> 
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    input.addEventListener('input', function() {
        ...
        var rows = document.querySelectorAll('.table tbody tr'); // Select all table rows
        var noCategoriesFound = true;

        rows.forEach(function(row) {
            var title = row.querySelector('td:first-child').textContent.toLowerCase(); // Get the text of the first column (Title)
            if (title.indexOf(inputValue) !== -1 || inputValue === '') {
                row.style.display = '';
        ...
        // Handle "No Categories Found" message
        var noCategoriesRow = document.querySelector('#noCategoriesRow');
        if (noCategoriesFound) {
            if (!noCategoriesRow) {
                var tableBody = document.querySelector('.table tbody');
                noCategoriesRow = document.createElement('tr');
                noCategoriesRow.id = 'noCategoriesRow';
                noCategoriesRow.innerHTML = '<td colspan="3" class="text-center">No Categories Found</td>';
