    ...
    <div class="container-fluid">
        ...
        <div class="card shadow border-0 mb-7">
            ...
            <div class="card-header add_cat">
                <h5 class="mb-0">Edit  {{$news->post_title}}</h5>
        ...
        <div class="card shadow border-0 mb-7 add-container">
            <div class="card-body">
                <form action="{{
                    route('admin.news.update',  [ 'id' => $news->id])
                }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                        <label for="title" class="form-label">Title:</label>
                        <input type="text" name="post_title" class="form-control" id="title" 
                        value="{{$news->post_title}}" required> 
                    </div>

                    <div class="mb-3">
                        <label for="content" class="form-label">Content:</label>
                        <textarea class="form-control" name="post_content" id="content"  rows="4" required>{{$news->post_content}}</textarea>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <label for="img" class="form-label">Or Input Image Url:</label>
                         ...
                         value="{{get_field('thumbnail','blog', $news->id, '')}}"
                         >
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control" id="featured" name="featured" required>
                            <option value="no" {{get_field('featured','blog', $news->id, 'no') == 'no' ? 'selected' : ''}}>No</option>
                            <option value="yes" {{get_field('featured','blog', $news->id, 'no') == 'yes' ? 'selected' : ''}}>Yes</option>   
                        </select>
                    </div>
